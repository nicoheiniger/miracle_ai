...
def filter_dates(extracted_dates, timestamp):
    ...
    # Ensure the timestamp itself is not included unless explicitly in extracted_dates
        ...
        try:
            ...
            if not pd.isna(parsed_date):  # Skip invalid dates
                ...
                if start_date <= parsed_date <= end_date:  # Check range
                    if parsed_date != pd.to_datetime(timestamp).tz_localize(None):  # Exclude timestamp itself
